...
- **設定檔 Google 雲端備份**: 主畫面右上角與右側都有專屬「備份設定檔」按鈕，可將帳號 Token 與應用程式設定加密上傳到 Google 雲端硬碟，換電腦後再還原
...
- **右上角與右側「備份設定檔」**：專屬按鈕，可把設定檔加密備份到 Google 雲端硬碟。
...
3. 輸入並確認四位數字備份密碼，按「加密並上傳設定檔至 Google 雲端硬碟」。每次建立獨立的 `.gmbak` 檔案，不覆寫舊備份。密碼不會儲存或上傳，忘記密碼無法還原。
